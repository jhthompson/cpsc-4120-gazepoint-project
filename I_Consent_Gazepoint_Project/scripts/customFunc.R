...
load.libraries <- function(libs){
	for (package in libs) {
		if (!require(package, character.only = TRUE, quietly = TRUE)) {
			install.packages(package, repos = "http://stat.ethz.ch/CRAN/", dependencies = TRUE)
...
##   between: a vector containing names of columns that contain grouping variables
##   na.rm: a boolean that indicates whether to ignore NA's
##   conf.interval: the percent range of the confidence interval (default is 95%)
summarySE <- function(data=NULL, dv, between=NULL, na.rm=TRUE,
    ...
    datac <- ddply(data, between, .drop=.drop,
...
## subject (identified by id) so that they have the same mean, within each group
## specified by between.
##   data: a data frame.
##   id: the name of a column that identifies each subject (or matched subjects)
##   dv: the name of a column that contains the variable to be summariezed
##   between: a vector containing names of columns that are between-subjects variables
##   na.rm: a boolean that indicates whether to ignore NA's
normDataWithin <- function(data=NULL, id, dv, between=NULL,
                           na.rm=FALSE, .drop=TRUE) {
    require(plyr)

    # Measure var on left, id + between vars on right of formula.
    data.subjMean <- ddply(data, c(id, between), .drop=.drop,
     .fun = function(xx, col, na.rm) {
    ...
    measureNormedVar <- paste(dv, "_norm", sep="")
    data[,measureNormedVar] <- data[,dv] - data[,"subjMean"] +
                               mean(data[,dv], na.rm=na.rm)
...
summarySEwithin <- function(data=NULL, dv, between=NULL, within=NULL,
                            id=NULL, na.rm=TRUE, conf.interval=.95, .drop=TRUE) {
  ...
  datac <- summarySE(data, dv, between=c(between, within),
  ...
  ndata <- normDataWithin(data, id, dv, between, na.rm, .drop=.drop)

  # This is the name of the new column
  dv_n <- paste(dv, "_norm", sep="")

  # Collapse the normed data - now we can treat between and within vars the same
  ndatac <- summarySE(ndata, dv_n, between=c(between, within),
                      na.rm=na.rm, conf.interval=conf.interval, .drop=.drop)
