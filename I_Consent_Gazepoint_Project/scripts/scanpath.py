...
#     t = (t - st)/1000000.0
      ...
      self.pupild.append(Point(d,d,t,err))
...
#   print xmlpp.pprint(tostring(exp))
  ...
  def getpcpd(self,fileName,w,h,bT,eT):
    ...
    for i in range(len(self.pupils)):
      ...
      if self.pupils[i].gettimestamp() < eT:
        ...
        self.pcpd.append(Point(pcpd,pcpd,t,'None'))
  ...
  def getbpcpd(self,fileName,w,h,bscanpath):
    ...
    for i in range(len(self.pupils)):
      ...
      t = self.pupils[i].gettimestamp()
      self.bpcpd.append(Point(pcpd,pcpd,t,'None'))
...
#   print "ICA: ", self.ICA
  ...
  def pscat(self,pupils):
    ...
    for pt in pupils:
      ...
      # (smoothed) data we are concatenating
      sd = pt.at(0)

      self.pupils.append(Point(sd,sd,pt.gettimestamp(),err))
...
#   self.pupils = bw.applyBWFilter(self.pupild,sfdegree,herz,sfcutoff)
    ...
    cf = 1.0/8.0*herz  # more smooth
#   cf = 1.0/16.0*herz # less smooth
    b, a = signal.butter(sfdegree,1.0/cf,'low',False,'ba')
    # construct 1D array
    d = []
    for pt in self.pupild:
      d.append(pt.at(0))
    sd = signal.filtfilt(b, a, d, padlen=len(d)-1)
    i=0
    for pt in self.pupild:
      err = pt.getStatus()
      self.pupils.append(Point(sd[i],sd[i],pt.gettimestamp(),err))
      i += 1
...
#     print "eta_y: ", eta_y
      ...
      print "te: ", te
      print "total time: ", tt
      print "fixation duration: ", self.fixations[k].getDuration()
